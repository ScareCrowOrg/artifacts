"""
Tests for png-generator-cell backend.
"""

import pytest
from unittest.mock import AsyncMock, Mock, patch
from artifacts.canonical.cell_types.png_generator_cell.backend.scripts import main


class TestExecuteCell:
    """Tests for execute_cell function."""
    
    def test_execute_cell_basic(self):
        """Test basic cell execution."""
        cell_data = {
            "prompt": "A red dragon",
            "generatedPng": None
        }
        
        result = main.execute_cell(cell_data)
        
        assert result["success"] is True
        assert result["message"] == "PNG generator cell ready"
        assert result["prompt"] == "A red dragon"
        assert result["has_png"] is False
    
    def test_execute_cell_with_png(self):
        """Test cell execution with generated PNG."""
        cell_data = {
            "prompt": "A blue crystal",
            "generatedPng": "data:image/png;base64,iVBORw0KGgoAAAANS..."
        }
        
        result = main.execute_cell(cell_data)
        
        assert result["success"] is True
        assert result["has_png"] is True
    
    def test_execute_cell_empty_prompt(self):
        """Test cell execution with empty prompt."""
        cell_data = {
            "prompt": "",
            "generatedPng": None
        }
        
        result = main.execute_cell(cell_data)
        
        assert result["success"] is True
        assert result["prompt"] == ""
        assert result["has_png"] is False


@pytest.mark.asyncio
class TestGeneratePngFromPrompt:
    """Tests for generate_png_from_prompt function."""
    
    async def test_generate_png_success(self):
        """Test successful PNG generation."""
        mock_service = AsyncMock()
        mock_service.generate_image.return_value = {
            "success": True,
            "image_base64": "iVBORw0KGgoAAAANS...",
            "metadata": {
                "prompt": "A blue crystal",
                "width": 512,
                "height": 512,
                "steps": 20,
                "cfg_scale": 7.0,
                "seed": 12345
            }
        }
        
        with patch('app.services.stable_diffusion_service.StableDiffusionService', 
                   return_value=mock_service):
            result = await main.generate_png_from_prompt(
                prompt="A blue crystal",
                width=512,
                height=512,
                steps=20,
                cfg_scale=7.0,
                seed=-1
            )
        
        assert result["success"] is True
        assert "image_base64" in result
        assert result["prompt"] == "A blue crystal"
        assert "metadata" in result
        
        # Verify service was called with correct parameters
        mock_service.generate_image.assert_called_once_with(
            prompt="A blue crystal",
            negative_prompt=None,
            width=512,
            height=512,
            steps=20,
            cfg_scale=7.0,
            seed=-1
        )
    
    async def test_generate_png_with_negative_prompt(self):
        """Test PNG generation with negative prompt."""
        mock_service = AsyncMock()
        mock_service.generate_image.return_value = {
            "success": True,
            "image_base64": "iVBORw0KGgoAAAANS...",
            "metadata": {}
        }
        
        with patch('app.services.stable_diffusion_service.StableDiffusionService',
                   return_value=mock_service):
            result = await main.generate_png_from_prompt(
                prompt="A landscape",
                negative_prompt="blurry, low quality"
            )
        
        assert result["success"] is True
        mock_service.generate_image.assert_called_once()
        call_args = mock_service.generate_image.call_args
        assert call_args.kwargs["negative_prompt"] == "blurry, low quality"
    
    async def test_generate_png_service_failure(self):
        """Test PNG generation when service returns failure."""
        mock_service = AsyncMock()
        mock_service.generate_image.return_value = {
            "success": False,
            "error": "Stable Diffusion API timeout"
        }
        
        with patch('app.services.stable_diffusion_service.StableDiffusionService',
                   return_value=mock_service):
            result = await main.generate_png_from_prompt(
                prompt="A mountain"
            )
        
        assert result["success"] is False
        assert "error" in result
        assert "timeout" in result["error"].lower()
    
    async def test_generate_png_exception_handling(self):
        """Test PNG generation exception handling."""
        mock_service = AsyncMock()
        mock_service.generate_image.side_effect = Exception("Connection error")
        
        with patch('app.services.stable_diffusion_service.StableDiffusionService',
                   return_value=mock_service):
            result = await main.generate_png_from_prompt(
                prompt="A forest"
            )
        
        assert result["success"] is False
        assert "error" in result
        assert "Connection error" in result["error"]
