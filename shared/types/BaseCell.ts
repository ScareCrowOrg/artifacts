...
export abstract class BaseCell {
  ...
  async show(data: Record<string, any>, options: ShowConfig): Promise<any> {
    ...
    if (hasCustomView) {
      // Custom view exists - return it for framework to render with context
      const cellTypeId = this.getCellTypeId()
      if (!cellTypeId) {
        throw new Error('Cannot determine cell type ID')
      }

      // Load the custom View component
      // Framework (useCellViewLoader) will call this method and get the component
      const cellType = await this.loadCellTypeFromDiscovery(cellTypeId)
      if (!cellType || !cellType.default_refs?.view?.[0]) {
        throw new Error(`No view component path for cell type: ${cellTypeId}`)
      }

      const componentPath = cellType.default_refs.view[0]
      // Return component info for framework to load
      return {
        cellType: cellTypeId,
        componentPath,
        cellInstance: this // Pass cell instance for View to use
      }
    }
  ...
  protected async checkViewComponentExists(): Promise<boolean> {
    try {
      ...
      console.debug('[BaseCell] Could not check view component existence:', error)
      return false
    }
  }
  ...
  protected async loadCellTypeFromDiscovery(cellTypeId: string): Promise<any> {
    try {
      // Attempt to dynamically import type.json from canonical artifacts
      // Path: artifacts/canonical/cell_types/{cellTypeId}/type.json
      // @vite-ignore - Template string import, resolved by browser import maps
      const typeModule = await import(
        `#artifacts/canonical/cell_types/${cellTypeId}/type.json`
      )
      return typeModule.default || typeModule
    } catch (error) {
      console.debug(`[BaseCell] Could not load type definition for ${cellTypeId}:`, error)
      return null
    }
  }
