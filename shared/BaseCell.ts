...
export abstract class BaseCell {
   ...
   * 2. If YES: Render it (cell handles its own UI)
   ...
   * ```
   */
  async show(data: Record<string, any>, options: ShowConfig): Promise<void> {
    // Default implementation: Generate dynamic form based on describe() schema
    // Framework will implement this logic
    // For now, this is a stub that cells can override
    
    // Check if custom View.vue exists (framework will implement)
    const hasCustomView = await this.checkViewComponentExists()
    
    if (hasCustomView) {
      // Custom view exists - framework handles rendering
      return
    }
